...
**专为 舞萌 DX (1.53 PRiSM) 打造的本地b50拉取 SDK**
...
- **防线穿透**：完整封装当前版本通讯逻辑对外提供极简的 C-ABI 接口
- **跨语言兼容**：提供标准极简的 C-ABI 接口，完美兼容 Python、C++、C# 等多语言的 FFI 调用。
- **内存安全**：内部独立维护线程安全的 Mutex 缓冲区，调用端无需手动管理指针与内存释放。
...
| `const char* infoApi()` | 获取玩家轻量级基础档案 | 返回 JSON 指针 (含昵称、rating、头像ID、姓名框ID) |
